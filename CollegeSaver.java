 ...
 * @author Killian Daly
 */
public class CollegeSaver extends Account {
    // Set variables to specific attributes of subclass object
    int time = 5;
    double intRate = .03, netInterest;
    double [] yearInterest = new double [time];
    
    public CollegeSaver(int accountNo, double balance, int time, double intRate) {
    ...
    public String accountInfo() {
        ...
        for (int i = 0; i < time; i++) {
            ...
            yearBreakdown = yearBreakdown + "Year " + (i + 1) + " @ 3% APR = " + euroFormat(yearInterest[i]) + "\n";
        }
        ...
        String accBreakdown = super.accountInfo() + "Account Type: College Saver" + "\n" + "Starting Balance: " + 
