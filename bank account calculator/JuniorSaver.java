 ...
 * @author Killian Daly
 */
public class JuniorSaver extends Account {
    // Set variables to specific attributes of subclass object
    int time = 3;
    double intRate = .02, netInterest;
    double [] yearInterest = new double [time];
    
    public JuniorSaver(int accountNo, double balance, int time, double intRate) {
    ...
    public String accountInfo() {
        ...
        for (int i = 0; i < time; i++) {
            ...
            yearBreakdown = yearBreakdown + "Year " + (i + 1) + " @ 2% APR = " + euroFormat(yearInterest[i]) + "\n";
        }
        ...
        String accBreakdown = super.accountInfo() + "Account Type: Junior Saver" + "\n" + "Starting Balance: " + 
